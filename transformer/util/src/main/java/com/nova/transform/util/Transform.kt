...
import com.nova.transform.kotlinx.redirect
...
fun ZipFile.transform(
    ...
    val creator = ParallelScatterZipCreator(ThreadPoolExecutor(CPU_NUM, CPU_NUM, 0L, TimeUnit.MILLISECONDS, LinkedBlockingQueue<Runnable>(), Executors.defaultThreadFactory(), RejectedExecutionHandler { runnable, _ ->
        runnable.run()
    }))
