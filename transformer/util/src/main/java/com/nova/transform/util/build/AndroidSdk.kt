package com.nova.transform.util.build

import com.nova.transform.kotlinx.isLinux
import com.nova.transform.kotlinx.isMac
import com.nova.transform.kotlinx.isWindows
import java.io.File
    ...
    companion object {
        ...
        val location: File
            get() = System.getenv("ANDROID_HOME")?.takeIf {
                ...
                isMac() -> File(HOME, "Library${File.separator}Android${File.separator}sdk").takeIf { it.exists() && it.isDirectory }
                isLinux() -> File(HOME, "Android${File.separator}sdk").takeIf { it.exists() && it.isDirectory }
                isWindows() -> File(HOME, "AppData${File.separator}Local${File.separator}Android${File.separator}sdk").takeIf { it.exists() && it.isDirectory }
                else -> null
            }
